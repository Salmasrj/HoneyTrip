...
data # Vérifiez la structure
data = data.frame(unclass(data), stringsAsFactors = TRUE)
for(i in 1:27) { data[,i] = as.factor(data[,i]) } # Transformer en facteurs
str(data) # Vérifiez que toutes les colonnes sont maintenant des facteurs
colnames(data)

    # Modele lineaire,
modellin2 <- glm(Destination_LM~ Sexe + Age + Dimache_Type+ Accessoire_LM, data = data, family = "binomial")
destination = data.frame(Sexe = "Femme", Age ="18-24 ans", Dimache_Type = "Netflix and chill", Accessoire_LM = "Une playlist privee pour des moments passionnes")
summary(modellin2)
predict(modellin2, newdata = destination, type = "response")
predict(modellin2, newdata = destination, type = "link")

    # Arbre 
arbre = rpart(Destination_LM ~ Sexe + Age + Dimache_Type + Accessoire_LM, data = data, method ="class", minsplit = 1, minbucket = 1)
rpart.plot(arbre)
# Differents arbres selon le parametre etudies
arbre_Dimache_Type = rpart(Dimache_Type~ ., data = data, method ="class")
rpart.plot(arbre_Dimache_Type)
printcp(arbre_Dimache_Type)
# Differents arbres selon le parametre etudies
arbre_Accessoire_LM = rpart(Accessoire_LM~ ., data = data, method ="class", minsplit = 1, minbucket = 1)
rpart.plot(arbre_Accessoire_LM, box.palette = "Blues")
printcp(arbre_Accessoire_LM)
...
results <- rfe(data[, -1], data$Destination_LM, sizes = c(1:5), rfeControl = control)
print(results)
